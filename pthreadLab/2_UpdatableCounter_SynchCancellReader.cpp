...
int main( int argc, char ** argv ){
	...
	cout << "Countdown finished, cancelling the reader thread\n";
	pthread_cancel( tid_reader );	// terminate reader thread after counter finished
...
void *inputReader( void *param ){
	...
	cout << "Type an integer value and press the Enter Key at any time: ";
	value = 10;	// change value to make countDownThread unblock and begin counting
	while( 1 ){
		pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, NULL );
		cin >> value;	// BLOCKING operation
		pthread_setcancelstate( PTHREAD_CANCEL_ENABLE, NULL );
		cout << "Checking for cancellation request\n";
		pthread_testcancel();	// DEFERRED CANCELLATION
	}
